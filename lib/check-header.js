...
const checkFileLicense = async(logger, file, licenseText, fix) => {
  ...
  matched = Comment.hasLicenseDeclaration(fileParseResult.comments);
  if (matched) {
  ...
  const fixedText = Comment.fixLicense(fileParseResult, matched, licenseText, commentDefinition);
 ...
 * @return {Boolean}
 */
const checkHeaderLicense = async(logger, folder, licenseFile, fix) => {
