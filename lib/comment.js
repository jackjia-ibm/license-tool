...
const hasExpectedLicense = (comments, licenseText) => {
  ...
  for (let comment of comments) {
    ...
    let expectedLicenseText = sanitizedLicenseText;
    // convert license years macro
    if (sanitizedLicenseText.indexOf(MACRO_LICENSE_YEAR) > -1) {
      let yearStart = comment.yearStart || currentYear;
      if (yearStart === currentYear) {
        expectedLicenseText = expectedLicenseText.replace(MACRO_LICENSE_YEAR, yearStart);
      } else {
        expectedLicenseText = expectedLicenseText.replace(MACRO_LICENSE_YEAR, `${yearStart}, ${currentYear}`);
      }
    }

    if (sanitizedComments === expectedLicenseText) {
      return comment;
    }
  }
...
const fixLicense = (fileParseResult, removeLicenses, licenseText, commentDefinition) => {
  ...
  if (removeLicenses) {
    // find the start year of the file
    let yearStart = currentYear;
    for (let removeLicense of removeLicenses) {
      if (removeLicense.yearStart && removeLicense.yearStart < yearStart) {
        yearStart = removeLicense.yearStart;
      }
    }
    // convert license years macro
